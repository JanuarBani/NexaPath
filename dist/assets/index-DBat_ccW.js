...
var P = (t, e, a) => e.has(t) || F('Cannot ' + a);
var d = (t, e, a) => (P(t, e, 'read from private field'), a ? a.call(t) : e.get(t)),
  m = (t, e, a) =>
    e.has(t)
      ...
      : e instanceof WeakSet
        ? e.add(t)
        : e.set(t, a),
  g = (t, e, a, n) => (P(t, e, 'write to private field'), n ? n.call(t, a) : e.set(t, a), a),
  M = (t, e, a) => (P(t, e, 'access private method'), a);
(function () {
  ...
  new MutationObserver((i) => {
    for (const r of i)
      if (r.type === 'childList')
        for (const l of r.addedNodes) l.tagName === 'LINK' && l.rel === 'modulepreload' && n(l);
    ...
    return [
      {
        ...
        description:
          'Belajar tentang pemrograman, sistem komputer, dan pengembangan perangkat lunak.',
        image: '/images/teknik-informatika.jpg',
      },
      {
        name: 'Kedokteran',
        description: 'Mempelajari ilmu kesehatan dan cara menangani pasien.',
        image: '/images/kedokteran.jpg',
      },
      {
        name: 'Desain Komunikasi Visual',
        description: 'Menggabungkan seni dan teknologi untuk menyampaikan pesan visual.',
      ...
      {
        name: 'Akuntansi',
        description: 'Mempelajari pencatatan dan pelaporan keuangan.',
        image: '/images/akuntansi.jpg',
      },
      {
        name: 'Hukum',
        description: 'Mempelajari sistem hukum dan penerapannya dalam masyarakat.',
        image: '/images/hukum.jpg',
      },
      {
        name: 'Manajemen',
        description: 'Belajar cara mengelola organisasi dan sumber daya secara efektif.',
        image: '/images/manajemen.jpg',
      },
      {
        name: 'Ilmu Komunikasi',
        description: 'Mempelajari cara berkomunikasi secara efektif dalam berbagai konteks.',
        image: '/images/ilmu-komunikasi.jpg',
      },
      {
        name: 'Farmasi',
        description: 'Mempelajari obat-obatan dan cara penggunaannya.',
        image: '/images/farmasi.jpg',
      },
      {
        name: 'Teknik Elektro',
        description: 'Belajar tentang sistem kelistrikan dan teknologi elektronik.',
        image: '/images/teknik-elektro.jpg',
      },
    ];
  }
}
...
class C {
  ...
  async loadCareer() {
    try {
      ...
      const e = await N.getTrendingCareersByIndustry();
      d(this, u).populateCareerList(e);
    } catch (e) {
      console.error('loadCareer error:', e),
        d(this, u).populateMajorListError('Gagal memuat daftar karier.');
    } finally {
      d(this, u).hideLoading();
    }
  }
  async loadMajor() {
    try {
      d(this, u).showLoading();
      const e = await H.getMajors();
      d(this, u).populateMajorList(e);
    } catch (e) {
      console.error('loadMajor error:', e),
        d(this, u).populateMajorListError('Gagal memuat daftar jurusan.');
    } finally {
      d(this, u).hideLoading();
    }
  }
...
<div id="career-major-section" class="w-full flex justify-center px-4 md:px-6 lg:px-8 py-8 bg-white dark:bg-gray-900">
  ...
  async afterRender() {
    try {
      this.showLoading(),
        g(this, b, new C({ view: this })),
        await d(this, b).init(),
        this.setupNavListeners();
    } catch (e) {
      console.error('afterRender: error saat inisialisasi:', e),
        this.showError('Gagal memuat data awal. Silakan coba lagi nanti.');
    } finally {
      this.hideLoading();
    }
    typeof I == 'function' ? I() : console.warn('setupScrollAnimation() tidak ditemukan.');
  }
  setupNavListeners() {
    const e = document.getElementById('careerNav'),
      a = document.getElementById('majorNav');
    e &&
      e.addEventListener('click', () => {
        this.setActiveTab('careerNav'), d(this, b).loadCareer();
      }),
      a &&
        a.addEventListener('click', () => {
          this.setActiveTab('majorNav'), d(this, b).loadMajor();
        });
  }
  setActiveTab(e) {
    document.querySelectorAll('.nav-btn').forEach((i) => {
      i.classList.remove('text-blue-600', 'border-blue-600'),
        i.classList.add('text-gray-700', 'dark:text-gray-300', 'border-transparent');
    });
    const n = document.getElementById(e);
    n &&
      (n.classList.add('text-blue-600', 'border-blue-600'),
      n.classList.remove('text-gray-700', 'dark:text-gray-300', 'border-transparent'));
  }
  showLoading() {
    const e = document.getElementById('content-loading-container');
    e &&
      (e.innerHTML = `
  ...
  showError(e) {
    ...
    a && (a.innerHTML = `<p class="text-red-600 font-semibold">${e}</p>`);
  }
  populateCareerList(e) {
    const a = document.getElementById('content');
    if (a) {
      if (!Array.isArray(e) || e.length === 0) {
        a.innerHTML =
          '<p class="text-gray-500 dark:text-gray-400 italic text-center mt-6">Belum ada data karier.</p>';
        return;
      }
      a.innerHTML = e
        .map(
          ({ industry: n, careers: i }) => `
...
`,
  ...
  populateMajorList(e) {
    ...
    if (a) {
      if (!Array.isArray(e) || e.length === 0) {
        a.innerHTML =
          '<p class="text-gray-500 dark:text-gray-400 italic text-center mt-6">Belum ada data jurusan.</p>';
        return;
      }
      a.innerHTML = `
    ...
    <div id="about-section" class="w-full min-h-screen px-4 md:px-6 lg:px-12 py-8 md:py-12 bg-white dark:bg-gray-900 flex flex-col space-y-12">
      ...
      <section aria-labelledby="techstack-heading"
        ...
        <div class="flex flex-wrap justify-center gap-6 text-gray-700 dark:text-gray-300">
          ${R.map(({ image: e, name: a, description: n }) => {
            const i = `desc-${a.replace(/\s+/g, '-').toLowerCase()}`;
            return `
...
async function q(t) {
  try {
    const e = await fetch('https://hapi-backend.vercel.app/api/prediksi', {
      method: 'POST',
    ...
    if (!e.ok) {
      ...
      throw new Error(a.error || `HTTP error! status: ${e.status}`);
    }
    return console.log(t), await e.json();
  } catch (e) {
    throw (console.error('API Error!', e), e);
  }
}
async function U(t) {
  const e = await fetch('https://hapi-backend.vercel.app/api/prediksi-jurusan', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(t),
  });
  if (!e.ok) {
    ...
    throw new Error(`HTTP error ${e.status}: ${n}`);
  }
  const a = await e.json();
  if (
    (console.log('Response API predictMajor:', a), !a || typeof a != 'object' || !a.recommendations)
  )
    throw new Error('Invalid response format from API');
  if (!Array.isArray(a.recommendations)) throw new Error('Recommendations should be an array');
  if (a.recommendations.length === 0) throw new Error('No recommendations found');
  return a;
}
class O {
  async render() {
  ...
  async afterRender() {
    const e = document.getElementById('recommendationForm'),
      a = document.getElementById('resultPage'),
      n = document.getElementById('recommendationResult'),
      i = document.getElementById('resetFormButton');
    e.addEventListener('submit', async (r) => {
      r.preventDefault();
      const l = {
        'Linguistic': parseFloat(e.Linguistic.value),
        'Musical': parseFloat(e.Musical.value),
        'Bodily': parseFloat(e.Bodily.value),
        'Logical - Mathematical': parseFloat(e.LogicalMathematical.value),
        'Spatial-Visualization': parseFloat(e.SpatialVisualization.value),
        'Interpersonal': parseFloat(e.Interpersonal.value),
        'Intrapersonal': parseFloat(e.Intrapersonal.value),
        'Naturalist': parseFloat(e.Naturalist.value),
        'math_score': parseFloat(e.math_score.value),
        'physics_score': parseFloat(e.physics_score.value),
        'biology_score': parseFloat(e.biology_score.value),
        'english_score': parseFloat(e.english_score.value),
        'history_score': parseFloat(e.history_score.value),
        'chemistry_score': parseFloat(e.chemistry_score.value),
        'geography_score': parseFloat(e.geography_score.value),
        'weekly_self_study_hours': parseFloat(e.weekly_self_study_hours.value),
        'absence_days': parseFloat(e.absence_days.value),
      };
      try {
        const o = await q(l),
          s = o.predicted_job || 'Tidak diketahui',
          h = (o.confidence * 100).toFixed(1);
        (n.innerHTML = `
        ...
        console.error(o), alert('Gagal memproses prediksi. Coba lagi.');
      }
    }),
      i.addEventListener('click', () => {
        e.reset(), e.classList.remove('hidden'), a.classList.add('hidden'), (n.innerHTML = '');
      });
  }
}
class V {
...
function G() {
  const t = document.getElementById('kelompok_ujian'),
    e = document.getElementById('tkaSection'),
    a = [
...
async function W() {
  const t = document.getElementById('predictionForm'),
    e = document.getElementById('resultPage'),
    a = document.getElementById('predictionResult'),
    n = document.getElementById('resetFormButton');
  t.addEventListener('submit', async (i) => {
    i.preventDefault();
    const r = new FormData(t),
      l = r.get('nama'),
      o = r.get('kelompok_ujian');
    if (!['saintek', 'soshum'].includes(o)) {
      alert('Pilih kelompok ujian yang valid.');
      return;
    }
    const s = {
      score_kpu: parseFloat(r.get('kpu')),
      score_kua: parseFloat(r.get('kua')),
      score_ppu: parseFloat(r.get('ppu')),
      score_kmb: parseFloat(r.get('kmb')),
    };
    if (
      (o === 'saintek'
        ? ((s.score_mat_tka = parseFloat(r.get('mat'))),
          (s.score_fis = parseFloat(r.get('fis'))),
          (s.score_kim = parseFloat(r.get('kim'))),
          (s.score_bio = parseFloat(r.get('bio'))))
        : ((s.score_mat_tka = parseFloat(r.get('mat'))),
          (s.score_geo = parseFloat(r.get('geo'))),
          (s.score_sej = parseFloat(r.get('sej'))),
          (s.score_sos = parseFloat(r.get('sos'))),
          (s.score_eko = parseFloat(r.get('eko')))),
      !Object.values(s).every((c) => typeof c == 'number' && !isNaN(c)))
    ) {
      alert('Pastikan semua nilai telah diisi dan berupa angka.');
      return;
    }
    const w = o === 'saintek' ? 'science' : 'humanities';
    try {
      const c = await U({ scores: s, test_type: w });
      if ((console.log('Result dari API:', c), !c || !Array.isArray(c.recommendations))) {
        alert('Data rekomendasi jurusan tidak tersedia atau formatnya salah.');
        return;
      }
      const j = c.recommendations
        .map(
          (p, E) => `
...
`),
      ...
      console.error(c), alert('Terjadi kesalahan saat memproses prediksi: ' + c.message);
    }
  }),
...
function Y(t) {
  let e = '';
  return t.resource && (e = e.concat(`/${t.resource}`)), t.id && (e = e.concat('/:id')), e || '/';
}
function Q() {
...
function ae({ skipTransition: t = !1, updateDOM: e }) {
  if (t || !document.startViewTransition || document.visibilityState !== 'visible') {
    const n = Promise.resolve().then(() => {
      e();
    });
    return {
      ready: Promise.resolve().catch(() => {}),
  ...
  return (
    a.ready.catch((n) => {
      n.name !== 'AbortError' && console.error('Error in transition.ready:', n);
    }),
    a.ready.then(() => {
      const n = document.querySelector('.active');
      n && n.classList.add('fade-out');
    }),
    a.finished
      .then(() => {
        const n = document.querySelector('.active');
        n && n.classList.add('fade-in');
      })
      .catch((n) => {
        n.name !== 'AbortError' && console.error('Error during transition.finished:', n);
      }),
    a
  );
}
...
var k, L, x, y, f, v, _;
class ne {
  constructor({ content: e, skipLinkButton: a }) {
    m(this, v);
    m(this, k);
    m(this, L);
    m(this, x, null);
    m(this, y, !1);
    m(this, f, null);
    g(this, k, e), g(this, L, a), ee(d(this, L), d(this, k));
  }
  async renderPage() {
    var e;
    if (!d(this, y)) {
      ...
      try {
        ...
        let i;
        if (
          (!n || typeof n != 'function' ? (i = new A()) : (i = n()),
          (e = d(this, x)) != null && e.destroy && d(this, x).destroy(),
          g(this, x, i),
          i === null)
        )
          return;
        if (typeof i.render != 'function') {
          d(this, k).innerHTML = '<p class="text-danger">Halaman tidak valid.</p>';
          return;
        }
        const r = await i.render(),
          l = document.querySelector('.active');
        l && (l.classList.add('inactive'), l.classList.remove('active'));
        const o = ae({
          updateDOM: () => {
            (d(this, k).innerHTML = r), d(this, f) && d(this, f).disconnect(), g(this, f, I());
            const s = document.querySelector('.page');
            s && (s.classList.add('active'), s.classList.remove('inactive'));
          },
        });
        o.ready.catch((s) => {
          s.name !== 'AbortError' && console.error('Transition Ready Error:', s);
        });
        try {
          await o.updateCallbackDone,
            scrollTo({ top: 0, behavior: 'instant' }),
            i.afterRender(),
            K() && (await M(this, v, _).call(this));
        } catch (s) {
          s.name !== 'AbortError' && console.error('Transition Done Error:', s);
        }
      } catch (a) {
        console.error('Render Page Error:', a);
      } finally {
        g(this, y, !1);
      }
    }
...
(k = new WeakMap()),
  ...
  (v = new WeakSet()),
  (_ = async function () {
    let e = document.getElementById('subscribe-button');
    if (!e || !e.parentNode) return;
    const a = await isCurrentPushSubscriptionAvailable(),
      n = e.cloneNode(!0);
    e.parentNode.replaceChild(n, e),
      (e = n),
      a
        ? (e.classList.remove('btn-warning', 'text-dark'),
          e.classList.add('btn-secondary', 'text-white'),
          (e.innerHTML = '<i class="fas fa-bell-slash"></i> Subscribed'),
          e.addEventListener('click', async () => {
            await unsubscribe(), await M(this, v, _).call(this);
          }))
        : (e.classList.remove('btn-secondary', 'text-white'),
          e.classList.add('btn-warning', 'text-white'),
          (e.innerHTML = '<i class="fas fa-bell"></i> Subscribe'),
          e.addEventListener('click', async () => {
            await subscribe(), await M(this, v, _).call(this);
          }));
  });
function $(t = !1) {
  const e = t
...
document.addEventListener('DOMContentLoaded', async () => {
  const t = document.getElementById('main-navbar'),
    e = document.getElementById('mobile-links');
  e && (e.innerHTML = $(!0)), t && (t.innerHTML = $(!1));
  const a = document.querySelector('#main-content'),
    n = document.getElementById('skip-link');
  if (!a || !n) {
    a || console.error("Elemen 'main-content' tidak ditemukan di DOM."),
      n || console.error("Elemen 'skip-link' tidak ditemukan di DOM.");
    return;
  }
  const i = document.getElementById('drawer-button'),
    r = document.getElementById('mobile-drawer'),
    l = document.getElementById('drawer-close'),
    o = document.getElementById('drawer-overlay');
  function s() {
    r && r.classList.add('-translate-x-full'), o && o.classList.add('hidden');
  }
  i == null ||
    i.addEventListener('click', () => {
      r && r.classList.remove('-translate-x-full'), o && o.classList.remove('hidden');
    }),
    l == null || l.addEventListener('click', s),
    o == null || o.addEventListener('click', s);
  const h = document.getElementById('theme-toggle'),
    w = document.getElementById('icon-sun'),
    c = document.getElementById('icon-moon');
  function j(S) {
    !w ||
      !c ||
      (S
        ? (w.classList.remove('hidden'), c.classList.add('hidden'))
        : (w.classList.add('hidden'), c.classList.remove('hidden')));
  }
  localStorage.getItem('theme') === 'dark'
    ? (document.documentElement.classList.add('dark'), j(!0))
    : (document.documentElement.classList.remove('dark'), j(!1)),
    h == null ||
      h.addEventListener('click', () => {
        const B = document.documentElement.classList.toggle('dark');
        localStorage.setItem('theme', B ? 'dark' : 'light'), j(B);
      });
  const E = new ne({ content: a, skipLinkButton: n });
  await te(),
    await E.renderPage(),
  ...
  T &&
    window.addEventListener('scroll', () => {
      T.classList.toggle('scrolled', window.scrollY > 10);
    });
});
function I() {
  const t = new IntersectionObserver(
    (e, a) => {
      e.forEach((n) => {
        n.isIntersecting &&
          (n.target.classList.add('animate-slide-up'),
          n.target.classList.remove('opacity-0'),
  ...
  return (
    document.querySelectorAll('[data-animate="slide-up"]:not(.animate-slide-up)').forEach((e) => {
      t.observe(e);
    }),
    t
  );
